...
const BuildingTable = React.memo(function BuildingTable({
    category,
    catIdx,
    defaultCategoryRows,
    updateRow,
    ...
    return (
        <div className="flex flex-col border rounded">
            <h3 className="font-semibold text-md mb-2 p-2">
                {category.category}
            </h3>
                ...
                <table className="w-full border text-sm">
                    ...
                    <tbody>
                        {category.rows.map((row, rowIdx) => {
                            ...
                            const isDefault = defaultTypesForCategory.includes(
                                row.type,
                            );
                            return (
                                <tr key={rowIdx}>
...
const FacilitiesServicesTable = React.memo(function FacilitiesServicesTable({
    category,
    catIdx,
    defaultCategoryRows,
    updateRow,
        ...
        <div className="flex flex-col border rounded">
            <div className="flex-1 overflow-auto">
                <table className="w-full border text-sm">
                    <thead className="bg-gray-100">
                        <tr>
                            <th className="border px-2 py-1 w-[400px]">
                                {category.category}
                            </th>
...
export default function Buildings() {
    ...
    useEffect(() => {
        setCraData((prev) => {
            ...
            next.facilities = defaultFacilities.map(
                (defaultCategory, index) => ({
                    ...defaultCategory,
                    ...(prev.facilities?.[index] || {}),
                    rows: prev.facilities?.[index]?.rows?.length
                        ? prev.facilities[index].rows
                        : defaultCategory.rows,
                }),
            );
    ...
    const updateBuildingRow = useCallback(
        (catIdx, rowIdx, field, value) => {
            setCraData((prev) => {
                ...
                if (
                    ...
                    !updatedBuildings[catIdx].rows ||
                    !updatedBuildings[catIdx].rows[rowIdx]
                ) {
                    console.error(
                        "Invalid indices for updateBuildingRow:",
                        catIdx,
                        rowIdx,
                    );
                    return prev;
                ...
                // Check if the current row's type is one of the default types for this category
                const isDefaultType = defaultRowsForCategory.some(
                    (defaultRow) =>
                        defaultRow.type ===
                        updatedBuildings[catIdx].rows[rowIdx].type,
                );
    ...
    const addBuildingRow = useCallback(
        (catIdx) => {
            setCraData((prev) => {
    ...
    const removeBuildingRow = useCallback(
        (catIdx, rowIdx) => {
            setCraData((prev) => {
                const updatedBuildings = [...prev.buildings]; // Create a new array
                // Safety check
                if (
                    ...
                    console.error(
                        "Invalid indices for removeBuildingRow:",
                        catIdx,
                        rowIdx,
                    );
                    return prev;
                ...
                const isDefault = defaultRowsForCategory.some(
                    (defaultRow) => defaultRow.type === rowToRemove.type,
                );
                ...
                const newRows = updatedBuildings[catIdx].rows.filter(
                    (_, i) => i !== rowIdx,
                );
                ...
                toast.error(
                    `Removed "${removed}" from "${updatedBuildings[catIdx].category}"`,
                );
                return { ...prev, buildings: updatedBuildings };
            });
        },
    ...
    const updateFacilityRow = useCallback(
        (catIdx, rowIdx, field, value) => {
            setCraData((prev) => {
                ...
                if (
                    ...
                    !updatedFacilities[catIdx].rows ||
                    !updatedFacilities[catIdx].rows[rowIdx]
                ) {
                    console.error(
                        "Invalid indices for updateFacilityRow:",
                        catIdx,
                        rowIdx,
                    );
                    return prev;
                ...
                const isDefaultType = defaultRowsForCategory.some(
                    (defaultRow) =>
                        defaultRow.type ===
                        updatedFacilities[catIdx].rows[rowIdx].type,
                );
    ...
    const addFacilityRow = useCallback(
        (catIdx) => {
            setCraData((prev) => {
                ...
                if (
                    categoryName === "Facilities and Services" ||
    ...
    const removeFacilityRow = useCallback(
        (catIdx, rowIdx) => {
            setCraData((prev) => {
                const updatedFacilities = [...prev.facilities]; // Create a new array
                // Safety check
                if (
                    ...
                    console.error(
                        "Invalid indices for removeFacilityRow:",
                        catIdx,
                        rowIdx,
                    );
                    return prev;
                ...
                // Create new rows array by filtering
                const newRows = updatedFacilities[catIdx].rows.filter(
                    (_, i) => i !== rowIdx,
                );
                ...
                toast.error(
                    `Removed "${removed}" from "${updatedFacilities[catIdx].category}"`,
                );
                return { ...prev, facilities: updatedFacilities };
            });
        },
    ...
    const isValidBuildings =
        ...
        craData.buildings.some(
            (cat) => Array.isArray(cat.rows) && cat.rows.length > 0,
        );

    const isValidFacilities =
        Array.isArray(craData.facilities) &&
        craData.facilities.length > 0 &&
        craData.facilities.some(
            (cat) => Array.isArray(cat.rows) && cat.rows.length > 0,
        );

    const buildingsData = isValidBuildings
        ? craData.buildings
        : defaultBuildings;
    const facilitiesData = craData.facilities || [];
    return (
        <div className="space-y-4">
            <Accordion title="D. Buildings and other Facilities in the Barangay">
                <p className="text-sm text-yellow-400 italic mb-2">
                    <strong>Note:</strong> Leave a cell blank if the value is
                    zero. Default building types cannot be removed or edited.
                </p>
            ...
            <Accordion title="E. Primary Facilities and Services in the Barangay">
                <p className="text-sm text-yellow-400 italic mb-2">
                    <strong>Note:</strong> Leave a cell blank if the value is
                    zero. Default facility types cannot be removed or edited.
                </p>
